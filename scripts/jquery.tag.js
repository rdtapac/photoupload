	...
	$.extend($.fn,{
		...
		showDrag: function(e){
			...
			var position = function(context){
				...
				object_positions = jtagdrag.position();

				left_pos = parseInt(object_positions.left) + border;
				top_pos =  parseInt(object_positions.top) + border;
			...
			$(".jTagSaveBtn",container).click(function(){
				...
				else
				{
					...
					{
						alert('label must be alphanumeric');
						return false;
					}
				}
				...
				object_parent_offset = $('.jTagDrag').position();

				top_pos = parseInt(object_parent_offset.top);
				left = parseInt(object_parent_offset.left);
